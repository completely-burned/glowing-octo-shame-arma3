...
TMPDIR=$(mktemp -td glowing-octo-shame-arma3.XXXXX)
echo $TMPDIR
...
if $DIAG_LOG; then
	DEBUGPOSTFIX=($(tar -cf - $TMPDIR/glowing-octo-shame-arma3/ | sha1sum))
...
for DIR in $(find $TMPDIR -maxdepth 1 -type d); do
	if [ -f "${DIR}/mission.sqm" ]; then
		...
		# cpmpat для a2 v1.11
		if [[ $NAME == *"compat"* ]]; then
			rsync --recursive --delete $TMPDIR/glowing-octo-shame-arma3/* $MISSION
			rsync --recursive --delete ${DIR}/* $MISSION
			find $MISSION -type f -exec sed -i "/^.*#define.*__A2OA__.*/d" {} \;
		else
			find $TMPDIR/glowing-octo-shame-arma3/ -mindepth 1 -maxdepth 1 -exec ln -sn {} $MISSION \;
