...
while {_cnt < _crewcount} do 
{
	...
	_name = name _unit;
	_icon = parseText "<img align='left' [cargo]/></t>";
	if(_unit == driver vehicle _unit) then 
	{
		_icon = parseText "<img align='left' [driver]/></t>"
	};	
	if(_unit == gunner vehicle _unit) then 
	{
		_icon = parseText "<img align='left' [gunner]/></t>"
	};	
	if(_unit == commander vehicle _unit) then 
	{
		_icon = parseText "<img align='left' [commander]/></t>"
	};	
