...
while {true} do {
	...
	if (_teleport) then {
		...
		if (isnil {_veh getvariable "_teleport_action"} && !isnull _veh) then {
			private ["_action"];
			_action = _veh addaction [localize "draga_str_teleportation", "m\functions\action_teleport.sqf", '#USER:teleport_0', 0.5, false, false, "","_target == player"];
	...
	if (_menu) then {
		...
		if (isnil {_veh getvariable "_menu_action"} && !isnull _veh) then {
			private ["_action"];
			_action = _veh addaction ["Menu", "m\client\main_menu.sqf", '#User:BIS_Menu_GroupCommunication', 0.5, false, false, "","_target == player"];
